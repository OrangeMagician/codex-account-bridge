...
CAB Desktop 使用 macOS UserDefaults 在本机保存服务器列表、当前选择、额度刷新间隔、额度周期唤醒设置与最近一次唤醒结果、“切换时保留项目与会话”偏好和它上次用于启动桌面客户端的 CAB 账号名称。每条服务器记录只包含随机 ID、用户填写的显示名称和 SSH 主机字符串。应用不保存 SSH 密码或私钥，也不读取 Codex 令牌内容。
...
- 额度周期唤醒默认关闭，只有用户主动确认后才会运行。唤醒使用所选账号的官方 Codex 可执行文件，在临时、只读、临时 Git 工作区发送固定的最小提示；工作区和模型响应不会保留。CAB 会先查询额度，仅在五小时或周周期没有倒计时时执行最小请求。暂停自动刷新时段会抑制普通自动读取和额度恢复触发的请求，用户显式设置的周期启动时间优先执行；失败不会自动重试。
