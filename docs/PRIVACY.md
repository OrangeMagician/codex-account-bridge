...
- 用户开启“切换时保留项目与会话”后，CAB 会在 Codex 完全停止时合并本地 `sessions/` 与 `archived_sessions/`，并在目标 `.codex-global-state.json` 中合并经过审核的本地项目、SSH 主机记录、远程项目路径、项目顺序、会话归属、会话标题与客户端绑定、侧栏显示状态、提示历史和未发送草稿。目标文件会先备份并原子写入。为让官方桌面侧栏保持一致，CAB 还会备份目标 `sqlite/codex-dev.db`，只合并经过结构校验的 `local_thread_catalog` 与主机目录行，并保留目标库的其他表和同步水位；同时对目标 `state_5.sqlite` 执行 checkpoint、完整性检查和 `backfill_state` 水位重置。CAB 不解释、展示或输出线程行、草稿、提示历史、会话正文或备份内容；SSH 私钥、账号身份、插件与 OAuth、设备标识、审批、沙箱和项目 trust 选择始终不从其他账号复制。
