...
- 用户开启“切换时保留项目与会话”后，CAB 会在 Codex 完全停止时合并本地 `sessions/` 与 `archived_sessions/`，并在目标 `.codex-global-state.json` 中合并经过审核的项目目录、顺序、当前项目和会话归属字段。目标文件会先备份并原子写入；身份、插件、提示词、实验和其他 UI 字段不从其他账号复制。为让官方桌面侧栏显示共享会话，CAB 只对目标 `state_5.sqlite` 执行 checkpoint、完整性检查和 `backfill_state` 水位重置；操作前在同一账号目录创建本地备份。CAB 不读取或输出线程行、会话正文或备份内容。
