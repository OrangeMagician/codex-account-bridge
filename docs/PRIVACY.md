...
CAB Desktop 使用 macOS UserDefaults 在本机保存服务器列表、当前选择、额度刷新间隔、“切换时保留会话”偏好和它上次用于启动桌面客户端的 CAB 账号名称。每条服务器记录只包含随机 ID、用户填写的显示名称和 SSH 主机字符串。应用不保存 SSH 密码或私钥，也不读取 Codex 令牌内容。
...
- 用户开启“切换时保留会话”后，CAB 会在 Codex 完全停止时合并本地 `sessions/` 与 `archived_sessions/`。为让官方桌面侧栏显示这些文件，CAB 只对目标 `state_5.sqlite` 执行 checkpoint、完整性检查和 `backfill_state` 水位重置；操作前在同一账号目录创建本地备份。CAB 不读取或输出线程行、会话正文或备份内容。
- 会话迁移预检只枚举名为 `codex` 的进程并读取其可执行文件路径，用于区分桌面端、编辑器扩展和 CLI；不读取进程命令参数、提示内容或环境变量，也不持久化完整路径。
