...
macOS 原生界面的“全局设置”提供可选的“额度重置通知”开关。开关默认关闭，仅在用户主动开启时申请系统通知权限；每次刷新额度后会替换 CAB 已安排的通知，关闭开关会撤销这些通知。通知只表示官方返回的重置时间已到，打开 CAB 刷新后可确认服务端最新额度。

自动额度刷新按账号独立判断：即使额度为 0，也会按设置间隔继续查询；手动刷新仍会查询全部账号。全局设置中的“额度周期唤醒”默认关闭，开启后可在额度恢复或最多 3 个周周期检查时间发送一次最低消耗的官方 Codex 请求，仅在周周期尚未开始时执行。最多 3 个免打扰时段会同时抑制自动额度查询和唤醒请求；唤醒后会再次查询并核对官方返回的重置时间，失败不会自动重试。
...
- “全局设置”还可以开启额度周期唤醒，配置最多 3 个周周期检查时间和 3 个免打扰时段。
...
“全局设置 > 项目与会话”中的“切换时保留”默认关闭。开启后，下一次桌面账号切换会在所有 Codex 进程停止后同步本地与远程项目、完整消息与会话目录、动态工具记录、长期目标、记忆、个人技能、规则、提示模板、附件、生成图片、侧栏状态、提示历史和未发送草稿，使多个登录身份共用同一套本地工作区体验。SSH 同步只复制官方 Codex 已保存的主机记录、远程路径和项目选择，不读取或复制私钥。写入前会逐项备份目标 JSON、目录和 SQLite；CAB 只合并经过列结构校验的白名单表，保留其他表和账号自己的处理水位，失败时自动回滚。账号身份、插件/OAuth、浏览器/Computer Use 配置、设备与远程控制授权、日志以及审批、沙箱和项目 trust 选择始终隔离，`auth.json` 和 `config.toml` 不共享、不读取、不复制。关闭该设置后，下一次切换会恢复每个账号独立的会话副本，项目列表也保持账号独立。
