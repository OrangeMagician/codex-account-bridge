...
自动额度刷新按账号独立判断：额度已耗尽且具有未来重置时间的账号会暂停请求到该次重置，其他账号仍按设置间隔正常刷新；手动刷新仍会查询全部账号。
...
“全局设置 > 项目与会话”中的“切换时保留”默认关闭。开启后，下一次桌面账号切换会在所有 Codex 进程停止后合并各账号的 `sessions/` 和 `archived_sessions/`，同步本地与远程项目、会话归属、完整会话目录、侧栏状态、提示历史和未发送草稿，因此切换后能继续看到相同项目、对话与正在编辑的内容。SSH 同步只复制官方 Codex 已保存的主机记录、远程路径和项目选择，不读取或复制私钥。写入前会备份原项目状态、`sqlite/codex-dev.db` 和 `state_5.sqlite`；CAB 只合并经过结构校验的会话目录行并保留数据库其他表，再重置官方 `backfill_state` 水位。账号身份、插件/OAuth、设备标识以及审批、沙箱和项目 trust 选择始终隔离，`auth.json` 和 `config.toml` 不共享、不读取、不复制。关闭该设置后，下一次切换会恢复每个账号独立的会话副本，项目列表也保持账号独立。
