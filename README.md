...
- “全局设置”集中管理 Codex 桌面端账号、跨账号会话共享和新任务自动轮换；这些策略作用于当前选择的整台 Mac 或 SSH 主机，不属于某个账号。
...
“全局设置 > 会话共享”中的“切换时保留”默认关闭。开启后，下一次桌面账号切换会在所有 Codex 进程停止后合并各账号的 `sessions/` 和 `archived_sessions/`，再让目标账号的官方 Codex 从这些会话文件重建自己的可见线程索引，因此切换后仍能看到未归档及已归档对话。CAB 在重置官方 `backfill_state` 水位前会执行 WAL checkpoint 和完整性检查，并把 `state_5.sqlite` 备份到同一账号目录；不会共享或复制 `auth.json`。关闭该设置后，下一次切换会恢复每个账号独立的会话副本。
